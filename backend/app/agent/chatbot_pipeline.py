...
from app.core.config import MAX_ATTEMPTS
...
def agent_node(state: AgentState) -> dict:
    # Convert LangChain messages to Gemini types.Content before calling API
...
# seller_id always injected from state — never from Gemini's arguments.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# PUBLIC ENTRY POINT
# Called by chat.py (the FastAPI route).
# ---------------------------------------------------------------------------
...
def run_chat(user_message: str, seller_id: str) -> str:
    ...
    config = {
        ...
        "recursion_limit": MAX_ATTEMPTS * 2
    }
