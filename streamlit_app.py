...
st.set_page_config(page_title="Retail Return Reasoning Agent", page_icon="📦", layout="wide")
...
def render_detail(d: dict):
    ...
    st.caption(f"Trend: {trend_icon} {trend.capitalize()} ({'+' if growth >= 0 else ''}{round(growth * 100, 1)}%)")
    st.divider()
    ...
    if evidence:
        ...
        if sku_breakdown:
            ...
            for sku in sku_breakdown:
                st.write(f"- {sku.get('variant') or sku.get('sku_id', '?')}: {sku.get('return_count', 0)} returns")
...
st.markdown(
    """
    <link href="https://fonts.googleapis.com/css2?family=Billabong&family=Grand+Hotel&family=Dancing+Script:wght@700&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet">
    <style>
        ...
        /* pull page content to top so topbar sits flush */
        .block-container {
        ...
        .rr-login-page [data-testid="stAppViewContainer"],
        ...
        /* push the columns container down so card is mid-screen */
        .rr-login-spacer {
            height: calc(50vh - 280px);
            min-height: 20px;
        }
        div[data-testid="stTextInput"] label:empty,
        div[data-testid="stTextInput"] label[data-testid="stWidgetLabel"]:has(p:empty) {
...
if not st.session_state.token:
    ...
    # Push card toward vertical center
    st.markdown('<div class="rr-login-spacer"></div>', unsafe_allow_html=True)

    # Center the card using 3 columns: narrow | card | narrow
...
# ── post-login: sidebar (backend URL + logout only) ────────────────────────────
...
dashboard_data, dashboard_error = api_request(base_url, "/dashboard", token=st.session_state.token)
...
col2.metric("High signals", sum(1 for item in products if item.get("return_signal") == "High"))
col3.metric("Normal/Low", sum(1 for item in products if item.get("return_signal") in {"Normal", "Low"}))
...
if not products:
    ...
    for product in products:
        with st.container(border=True):
            ...
            with left:
                st.markdown(f"**{product.get('product_name', product.get('product_id', 'Unknown product'))}**")
...
(function() {{
  ...
  if (doc.getElementById("rr-chat-fab")) return;
  ...
  style.textContent = `
    ...
    #rr-chat-header-icon {{ font-size: 18px; }}
    ...
    #rr-chat-textarea {{
      ...
      outline: none; overflow-y: auto; transition: border-color 0.15s;
    }}
    ...
    #rr-chat-send {{
      ...
      transition: background 0.15s, transform 0.1s; margin-bottom: 1px;
    }}
  ...
  panel.innerHTML = `
    <div id="rr-chat-header">
      <div id="rr-chat-header-left">
        <span id="rr-chat-header-icon">🤖</span>
    ...
    <div id="rr-chat-body">
      <div class="rr-chat-msg bot">Hi! Ask me anything about your product returns 📦</div>
    </div>
  ...
  function appendMessage(text, role) {{
    ...
    div.className = "rr-chat-msg " + role;
  ...
  async function sendMessage() {{
    ...
    try {{
      ...
      thinking.remove();
      if (!response.ok) {{ appendMessage("Error: " + (data.detail || "Something went wrong."), "bot"); return; }}
